    ...
    <div class="row mb-3">
        ...
        <div class="col-md-4">
            ...
            <select id="order_status" class="form-control showBonds" name="status">
                ...
                <option @isset($request['status']) @if($request['status']=='under_implementation') selected @endif   @endisset value="under_implementation">تحت التنفيذ</option>
                <option @isset($request['status']) @if($request['status']=='delaying') selected @endif   @endisset value="delaying">مؤجل </optiيذon>

                <option @isset($request['status']) @if($request['status']=='cancel') selected @endif   @endisset value="cancel">لاغي </optiيذon>


                <option @isset($request['status']) @if($request['status']=='collection') selected @endif   @endisset value="collection">التحصيل</option>
                <option @isset($request['status']) @if($request['status']=='paid') selected @endif   @endisset value="paid">الدفع</option>
       ...
       if(from_date=='' || to_date== '' )
       {
        ...
        return;
                   ...
                   $.ajax({
                        ...
                        type: 'POST',
                        data: {delivery_id: delivery_id ,from_date : from_date,to_date:to_date,order_status:order_status},
                        beforeSend: function () {

<form id="form" enctype="multipart/form-data" method="POST"
    action="{{ route('admin.changeStatusForOrder_store', $order->id) }}">
    @csrf
    <div class="row g-4">
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <select class="form-control" name="status" id="status-convert" onchange="put_partial_value();">
                ...
                <option value="total_delivery_to_customer" @if ($order->status == 'total_delivery_to_customer') selected @endif>تم التسليم
                </option>
                <option value="partial_delivery_to_customer" @if ($order->status == 'partial_delivery_to_customer') selected @endif>تسليم
                    جزئي</option>
                ...
                <option value="under_implementation" @if ($order->status == 'under_implementation') selected @endif> تحت التنفيذ
                </option>
                <option value="shipping_on_messanger" @if ($order->status == 'shipping_on_messanger') selected @endif> الشحن علي
                    الراسل</option>
        ...
        <div class=" col-sm-6 partial_value" @if ($order->status != 'partial_delivery_to_customer') style="display: none;" @endif>
            ...
            <input type="number" value="{{ $order->partial_value }}" name="partial_value" class="form-control"
                id="partial_value" />
        </div>

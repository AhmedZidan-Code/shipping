...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('agents.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            <!--begin::Label-->
            <label for="user_name" class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
                <span class="required mr-1"> طريقه استحقاق الراتب</span>
            </label>
            <!--end::Label-->
            @php $type_paid = array(1=>'راتب شهري'
            ,2=>'راتب شهري وعموله',
            3=>'عموله فقط') @endphp

...
class UnderImplementationOrderController extends Controller
{
    //
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $admins = Order::query()->where('trader_id',  $trader->id)->with(['province', 'trader', 'delivery'])->where('status', 'under_implementation')->orderBy('updated_at', 'desc');
            ...
            $this->add_log_activity(null, auth('trader')->user(), "تم عرض  الطلبات تحت التنفيذ ");

        }
        return view('Trader.Orders.underImplementationOrders.index');
    }
}

...
$trader = auth('trader')->user();
        ...
        if ($request->ajax()) {
            $admins = Order::query()->where('trader_id', $trader->id)->with(['province', 'trader', 'delivery'])->where('status', 'shipping_on_messanger')->orderBy('updated_at', 'desc');
        ...
        return view('Trader.Orders.shippingOnMessanger.index');
    }
}

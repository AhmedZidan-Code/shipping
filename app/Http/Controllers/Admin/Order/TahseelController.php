...
class TahseelController extends Controller
{
    ...
    public $statusArray = array('total_delivery_to_customer', 'partial_delivery_to_customer');
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax() && $request->has('trader_id')) {
    ...
    public function get_tahseel(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($rows->where('status', 'partial_delivery_to_customer')->exists()) {
                $totalShipmentValue = $rows->sum('partial_value');
            } else {
                $totalShipmentValue = $rows->sum('total_value');
            }

            $this->add_log_activity(null, auth('admin')->user(), "تم عرض تقارير التجار");

            return view('Admin.CRUDS.Orders.tahseel.get_tahseel', compact('request', 'traders', 'totalShipmentValue'));
        }
    }

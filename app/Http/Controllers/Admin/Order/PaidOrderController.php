...
class PaidOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $admins = Order::query()->with(['province', 'trader', 'delivery'])->where('status', 'paid')->orderBy('updated_at', 'desc');
...
//                    return $status;
            ...
            $this->add_log_activity(null, auth('admin')->user(), "تم عرض  الطلبات المدفوعة ");

        }
        $total_value=Order::where('status', 'paid')->sum('total_value');
        return view('Admin.CRUDS.Orders.paidOrders.index',compact('total_value'));
    }
